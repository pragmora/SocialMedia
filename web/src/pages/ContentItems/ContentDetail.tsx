...
export default function ContentDetail() {
  ...
  const backTarget = returnTo || '/dashboard/content-items'
  ...
  const canDeleteContent = can(user, 'content', 'delete')
  ...
  if (!item) {
    return (
      <div className="max-w-3xl mx-auto p-4">
        <div role="alert" className="rounded-xl border border-red-200 dark:border-red-900/60 bg-red-50 dark:bg-red-950/40 px-5 py-4 text-sm text-red-700 dark:text-red-300">{error || t('content.notFound')}</div>
        <Link to="/dashboard/content-items" className="text-socialflow-600 dark:text-socialflow-400 hover:underline text-sm mt-3 inline-block">
  ...
  return (
    <div className="max-w-3xl mx-auto animate-fade-in">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800 p-5 md:p-7 mb-5 shadow-sm">
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-5">
          <div className="flex-1 min-w-0">
            <h1 className="text-2xl md:text-3xl font-bold text-slate-900 dark:text-slate-100 leading-tight">{item.title}</h1>
            <div className="flex flex-wrap items-center gap-2 mt-2.5">
              ...
              <span className="text-xs text-slate-500 dark:text-slate-400">{getPlatformLabel(item.platform)}</span>
              <span className="text-xs text-slate-400">·</span>
              <span className="text-xs text-slate-500 dark:text-slate-400">{getContentTypeLabel(item.content_type)}</span>
            </div>
          </div>
          <div className="flex items-center gap-2 self-start shrink-0">
            {canUpdateContent && (
              <Link
                to={returnTo ? `/dashboard/content-items/${item.id}/edit?return_to=${encodeURIComponent(returnTo)}` : `/dashboard/content-items/${item.id}/edit`}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
          {item.scheduled_date && (
            <div className="rounded-xl bg-slate-50 dark:bg-slate-800/50 px-3.5 py-2.5">
              ...
              <p className="text-sm font-medium text-slate-800 dark:text-slate-200 mt-0.5">{item.scheduled_date}</p>
            </div>
          )}
          {item.fecha_inicial && (
            <div className="rounded-xl bg-slate-50 dark:bg-slate-800/50 px-3.5 py-2.5">
              <p className="text-[11px] font-medium text-slate-400 uppercase tracking-wider">{t('content.startDate')}</p>
              <p className="text-sm font-medium text-slate-800 dark:text-slate-200 mt-0.5">{item.fecha_inicial}</p>
            </div>
          )}
          {item.fecha_final && (
            <div className="rounded-xl bg-slate-50 dark:bg-slate-800/50 px-3.5 py-2.5">
              <p className="text-[11px] font-medium text-slate-400 uppercase tracking-wider">{t('content.endDate')}</p>
              <p className="text-sm font-medium text-slate-800 dark:text-slate-200 mt-0.5">{item.fecha_final}</p>
            </div>
          )}
          <div className="rounded-xl bg-slate-50 dark:bg-slate-800/50 px-3.5 py-2.5 min-w-0">
            ...
            <p className="text-sm font-medium text-slate-800 dark:text-slate-200 mt-0.5 truncate">
              {item.assignee_id
                ? (membersList.find((m) => m.user_id === item.assignee_id)?.user.name ||
                   membersList.find((m) => m.user_id === item.assignee_id)?.user.email ||
                   item.assignee_id)
                : '—'}
            </p>
          </div>
        ...
        {canUpdateContent && allowedTransitions.length > 0 && (
            ...
            <div className="flex flex-wrap gap-2">
              {allowedTransitions.map((next) => (
                <button
                  ...
                  className="rounded-lg border border-slate-200 dark:border-slate-700 px-3.5 py-2 text-xs font-medium text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800 hover:border-slate-300 dark:hover:border-slate-600 transition-all disabled:opacity-50 active:scale-[0.97]"
                >
                  {getStatusLabel(next)}
                </button>
              ))}
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800 p-5 md:p-7 mb-5 shadow-sm">
        ...
        {canCreateTask && (
          <form onSubmit={handleCreateTask} className="flex gap-2 mb-5">
            <input
              ...
              className="flex-1 rounded-lg border border-slate-200 dark:border-slate-700 px-3 py-2 text-sm focus:border-socialflow-500 focus:outline-none focus:ring-1 focus:ring-socialflow-500"
            />
            <button
              type="submit"
        ...
        {tasksLoading ? (
          ...
          <div className="space-y-1.5">
            {tasks.map((task) => (
              <div key={task.id} className="flex items-center gap-3 rounded-xl bg-slate-50 dark:bg-slate-800/50 px-3.5 py-2.5 hover:bg-slate-100/60 transition-colors group">
                {canUpdateTask ? (
                  <button
                ...
                <span className={`flex-1 text-sm ${task.done ? 'line-through text-slate-400' : 'text-slate-700 dark:text-slate-200'}`}>
                  <Link to={returnTo ? `/dashboard/tasks/${task.id}?return_to=${encodeURIComponent(returnTo)}` : `/dashboard/tasks/${task.id}`} className="hover:text-socialflow-600 dark:hover:text-socialflow-400">
                    {task.title}
                  </Link>
                </span>
                {task.end_date && (
                  <span className="text-xs text-slate-400">{task.end_date}</span>
                )}
              </div>
            ))}
          </div>
      ...
      {/* Comments section */}
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800 p-5 md:p-7 shadow-sm">
        <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100 mb-4">
        ...
        {canUpdateContent && (
          <form onSubmit={handleAddComment} className="mb-6">
            <textarea
              ...
              rows={2}
              className="w-full rounded-xl border border-slate-200 dark:border-slate-700 px-3.5 py-2.5 text-sm focus:border-socialflow-500 focus:outline-none focus:ring-1 focus:ring-socialflow-500 resize-none"
            />
        ...
        {(!item.comments || item.comments.length === 0) ? (
          ...
          <div className="space-y-3">
            {item.comments.map((c) => (
              <div key={c.id} className="rounded-xl bg-slate-50 dark:bg-slate-800/50 px-4 py-3 border border-slate-100/60">
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-center gap-2 min-w-0">
                    <div className="w-6 h-6 rounded-full bg-socialflow-100 dark:bg-socialflow-900/40 flex items-center justify-center text-socialflow-700 dark:text-socialflow-300 text-[10px] font-semibold shrink-0">
                ...
                <p className="mt-1.5 text-sm text-slate-600 dark:text-slate-300 leading-relaxed">{c.body}</p>
              </div>
            ))}
          </div>
        )}
